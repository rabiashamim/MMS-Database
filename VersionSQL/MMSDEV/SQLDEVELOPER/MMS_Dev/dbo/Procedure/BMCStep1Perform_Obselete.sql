/****** Object:  Procedure [dbo].[BMCStep1Perform_Obselete]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- Description:               
--==========================================================================================
CREATE   PROCEDURE dbo.BMCStep1Perform_Obselete 
...
SELECT 
	MAX(CASE WHEN ((@vYear BETWEEN YEAR(rrv.RuReferenceValue_EffectiveFrom) AND YEAR(rrv.RuReferenceValue_EffectiveTo)) AND srt.SrReferenceType_Name = 'Reserve Margin')
	THEN rrv.RuReferenceValue_Value ELSE 0 END) AS RM

	,MAX(CASE WHEN ((@vYear BETWEEN YEAR(rrv.RuReferenceValue_EffectiveFrom) AND YEAR(rrv.RuReferenceValue_EffectiveTo)) AND srt.SrReferenceType_Name = 'RE')
	THEN rrv.RuReferenceValue_Value ELSE 0 END) AS RE

	,MAX(CASE WHEN ((@vYear BETWEEN YEAR(rrv.RuReferenceValue_EffectiveFrom) AND YEAR(rrv.RuReferenceValue_EffectiveTo)) AND srt.SrReferenceType_Name = 'LIC')
	THEN rrv.RuReferenceValue_Value ELSE 0 END) AS LIC

	,MAX(CASE WHEN ((@vYear BETWEEN YEAR(rrv.RuReferenceValue_EffectiveFrom) AND YEAR(rrv.RuReferenceValue_EffectiveTo)) AND srt.SrReferenceType_Name = 'KE Share')
	THEN rrv.RuReferenceValue_Value ELSE 0 END) AS KEShare
...
FROM  MMS_Dev.dbo.LuAllocationFactors
...
BEGIN
	...
	, [MtStatementProcess_ID])
		...
		SELECT
			AVG(BMCAvailableCapacityGUHourly_CriticalHourCapacity) AS AvgCapacityCal
		   ,BMCAvailableCapacityGUHourly_SoUnitId
		   ,MtGenerator_Id
		   ,MtGenerationUnit_Id
		   ,MtStatementProcess_ID
