...
if(@pStepId=1)
BEGIN
select  ROW_NUMBER() OVER(Order by BmeStatementData_Id) as [Sr],  BmeStatementData_Month as [Month], BmeStatementData_Day as [Day], BmeStatementData_Hour as [Hour], BmeStatementData_CdpId as [CDP-ID], BmeStatementData_FromPartyRegisteration_Name as [Connected From], BmeStatementData_ToPartyRegisteration_Name as [Connected To], BmeStatementData_FromPartyRegisteration_Id as [Connected From Id], BmeStatementData_ToPartyRegisteration_Id as [Connected To Id], BmeStatementData_LineVoltage as [Line Voltage (kV)], BmeStatementData_IncEnergyImport as [Energy Import (kWh)], BmeStatementData_IncEnergyExport  as [Energy Export (kWh)] from BmeStatementDataCdpHourly_SettlementProcess
...
Order by BmeStatementData_Month, BmeStatementData_Day, BmeStatementData_Hour, BmeStatementData_CdpId
;
END
...
---------- 2.2
select  ROW_NUMBER() OVER(Order by BmeStatementData_Id) as [Sr],  BmeStatementData_Month as [Month], BmeStatementData_Day as [Day], BmeStatementData_Hour as [Hour], BmeStatementData_CdpId as [CDP-ID], BmeStatementData_FromPartyRegisteration_Name as [Connected From], BmeStatementData_ToPartyRegisteration_Name as [Connected To], BmeStatementData_FromPartyRegisteration_Id as [Connected From Id], BmeStatementData_ToPartyRegisteration_Id as [Connected To Id], BmeStatementData_LineVoltage as [Line Voltage (kV)], BmeStatementData_IncEnergyImport as [Energy Import (kWh)], BmeStatementData_IncEnergyExport  as [Energy Export (kWh)] , BmeStatementData_AdjustedEnergyImport as [Adjusted Energy Import (kWh)],
...
BmeStatementData_Hour as [Hour], BmeStatementData_SellerPartyRegisteration_Name as [Seller Name], BmeStatementData_BuyerPartyRegisteration_Name as [Buyer Name], BmeStatementData_SellerPartyRegisteration_Id as [Seller Id], BmeStatementData_BuyerPartyRegisteration_Id as [Buyer Id],
...
Insert into #tempStep10([Month],[MP Name],[Amount Payable / Amount Receivable (PKR)],
[Imbalance Charges (PKR)]) Values(
'',
'Total',(select sum([Amount Payable / Amount Receivable (PKR)]) from #tempStep10),(select sum([Imbalance Charges (PKR)]) from #tempStep10) )

select [Sr]	,case WHEN [Month]=0 THEN NULL else [Month] end	as [Month]	,[MP Name]	,[MP ID], [Actual Energy (kW)] as	 [Actual Energy (kW)],[Adjusted Energy (kW)] AS [Adjusted Energy (kW)],
[Energy Supplied (kW)],[Settlement of Legacy (PKR)],[Imbalance Charges (PKR)],[Amount Payable / Amount Receivable (PKR)] from #tempStep10 
...
(select SUM(MPH.BmeStatementData_ActualEnergy_Metered) from BmeStatementDataMpHourly_SettlementProcess MPH 
...
into #tempStep11
 from BmeStatementDataMpMonthly_SettlementProcess MPM
 Join  BmeStatementDataMpMonthly_SettlementProcess MPM_Previous on MPM.BmeStatementData_PartyRegisteration_Id=MPM_Previous.BmeStatementData_PartyRegisteration_Id 
where MPM.BmeStatementData_StatementProcessId=@pSettlementProcessId
and MPM_Previous.BmeStatementData_StatementProcessId=@vPredecessorId
--and MPM.BmeStatementData_PartyRegisteration_Id<>1115
Order by MPM.BmeStatementData_PartyRegisteration_Id;

Insert into #tempStep11([MP Name],[Month],[Previous Month Amount Payable / Amount Receivable (PKR)],[Amount Payable / Amount Receivable (PKR)],[ESS Adjustment (PKR)],[Imbalance Charges (PKR)]) 
Values('Total',
'',
	(select sum([Previous Month Amount Payable / Amount Receivable (PKR)]) from #tempStep11),
...
set @query='select
	[Sr]	,case WHEN [Month]=0 THEN NULL else [Month] end	as [Month]	,[MP Name]	,[MP ID],	[Actual Energy] as [Actual Energy (kW)],[Settlement of Legacy (PKR)],[Imbalance Charges (PKR)],[Amount Payable / Amount Receivable (PKR)]	,[Previous Month Amount Payable / Amount Receivable (PKR)] as ['+@vPredecessorMonthName+'],	[ESS Adjustment (PKR)] from #tempStep11 order by case when [Sr] is null then 1 else 0 end, [Sr]
';
exec (@query);
END
