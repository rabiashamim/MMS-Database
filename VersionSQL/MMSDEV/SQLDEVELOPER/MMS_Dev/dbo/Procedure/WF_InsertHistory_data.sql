...
CREATE procedure WF_InsertHistory_data                                        
...
and isnull(is_deleted,0)!=1               
     ...
     case when (@WF_status='WFSM' and @level_id != @interface_chain_count ) or (@WF_status='WFAP' and @level_id<@interface_chain_count) -- in('WFSM','WFAP')                    
              then  @level_id + 1                                          
            else                 
                @level_id                                          
          end                  
...
update MtWFHistory                                          
    set MtWFHistory_ToResource=@initiator--null
	 ,notify_flag=1
    where RuWorkFlowHeader_id = @WorkFlowHeader_id                                          
          and MtWFHistory_Process_id = @ProcessId                                   
    and MtWFHistory_SequenceID=@sequence_id                                  
