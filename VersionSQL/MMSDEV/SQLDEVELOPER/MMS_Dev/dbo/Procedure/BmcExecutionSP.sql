/****** Object:  Procedure [dbo].[BmcExecutionSP]    Committed by VersionSQL https://www.versionsql.com ******/
...
CREATE PROCEDURE dbo.BmcExecutionSP @pStatementProcessId DECIMAL(18, 0),
@pYear VARCHAR(40),
@pUserId INT
...
BEGIN
	...
	BEGIN TRY
		...
		SET @output = 'Process Execution Started: ' + @name + ', Settlement Period:' + CONVERT(VARCHAR(MAX), @pYear)
		--SELECT  
		-- @output;  
		...
		BEGIN
			...
			SET @vDescription = CONCAT('Step # 13 - BMC Step 13 started at ', FORMAT(GETDATE(), 'dd-MMM-yyyy HH:mm tt', 'en-US'));
			SELECT
				@vRuStepDefId = RuStepDef_ID
			FROM RuStepDef
			WHERE SrProcessDef_ID = @vProcessDefId
			AND RuStepDef_BMEStepNo = 13
			...
			EXEC [BMCEYSSStep13Perform] @pStatementProcessId

			SET @vDescription = CONCAT('Step # 13 - BMC Step 13 completed at ', FORMAT(GETDATE(), 'dd-MMM-yyyy HH:mm tt', 'en-US'));
			SELECT
				@vRuStepDefId = RuStepDef_ID
			FROM RuStepDef
			WHERE SrProcessDef_ID = @vProcessDefId
		...
		BEGIN
			DECLARE @pProcessName VARCHAR(MAX) = '';
			IF @vProcessDefId = 16
			BEGIN
				SET @pProcessName = 'PYSS';
			END
			ELSE
			BEGIN
				SET @pProcessName = 'FYSS';
			END


			SET @vDescription = CONCAT('BMC-', @pProcessName, 'Process execution for year ', @pYear, ' begins at ', FORMAT(GETDATE(), 'dd-MMM-yyyy HH:mm tt', 'en-US'));
			...
			SET @vDescription = CONCAT('Step # 0 - BMC-', @pProcessName, ' completed at ', FORMAT(GETDATE(), 'dd-MMM-yyyy HH:mm tt', 'en-US'));
			...
			SET @vDescription = CONCAT('Step # 1 - BMC-', @pProcessName, ' Step 1 started at ', FORMAT(GETDATE(), 'dd-MMM-yyyy HH:mm tt', 'en-US'));
			SELECT
				@vRuStepDefId = RuStepDef_ID
			FROM RuStepDef
			WHERE SrProcessDef_ID = @vProcessDefId
			...
			SET @vDescription = CONCAT('Step # 1 - BMC-', @pProcessName, ' Step 1 completed at ', FORMAT(GETDATE(), 'dd-MMM-yyyy HH:mm tt', 'en-US'));
			SELECT
				@vRuStepDefId = RuStepDef_ID
			FROM RuStepDef
			WHERE SrProcessDef_ID = @vProcessDefId
			...
			SET @vDescription = CONCAT('Step # 2 - BMC-', @pProcessName, ' Step 2 started at ', FORMAT(GETDATE(), 'dd-MMM-yyyy HH:mm tt', 'en-US'));
			SELECT
				@vRuStepDefId = RuStepDef_ID
			FROM RuStepDef
			WHERE SrProcessDef_ID = @vProcessDefId
			...
			SET @vDescription = CONCAT('Step # 2 - BMC-', @pProcessName, ' Step 2 completed at ', FORMAT(GETDATE(), 'dd-MMM-yyyy HH:mm tt', 'en-US'));
			SELECT
				@vRuStepDefId = RuStepDef_ID
			FROM RuStepDef
			WHERE SrProcessDef_ID = @vProcessDefId
			...
			SET @vDescription = CONCAT('Step # 3 - BMC-', @pProcessName, ' Step 3 started at ', FORMAT(GETDATE(), 'dd-MMM-yyyy HH:mm tt', 'en-US'));
			SELECT
				@vRuStepDefId = RuStepDef_ID
			FROM RuStepDef
			WHERE SrProcessDef_ID = @vProcessDefId
			...
			SET @vDescription = CONCAT('Step # 3 - BMC-', @pProcessName, ' Step 3 completed at ', FORMAT(GETDATE(), 'dd-MMM-yyyy HH:mm tt', 'en-US'));
			SELECT
				@vRuStepDefId = RuStepDef_ID
			FROM RuStepDef
			WHERE SrProcessDef_ID = @vProcessDefId
			...
			SET @vDescription = CONCAT('BMC-', @pProcessName, ' Process completed at ', FORMAT(GETDATE(), 'dd-MMM-yyyy HH:mm tt', 'en-US'));
			EXEC [dbo].[SaveMtSattlementProcessLogs] @StatementProcessId = @pStatementProcessId
													,@pRuStepDef_ID = NULL
													,@LogsMessage = @vDescription
													,@OutputMessage = 'Success'
		...
		----------------------------------  
		--SET @pCurrentSettlementPeriodId1 = [dbo].[GetSettlementMonthYear](@pCurrentSettlementPeriodId)  
		SET @output = 'Process Execution Completed: ' + @name + ', Settlement Period:' + CONVERT(VARCHAR(MAX), @pYear)
	...
	BEGIN CATCH
		...
		----------------------------------  
		SET @pCurrentSettlementPeriodId1 = [dbo].[GetSettlementMonthYear](@pCurrentSettlementPeriodId)
		SET @output = 'Process Execution Interrupted: ' + @name + ', Settlement Period:' + CONVERT(VARCHAR(MAX), @pYear)
		SELECT
			@output;
		EXEC [dbo].[SystemLogs] @user = @pUserId
							   ,@moduleName = 'Settlements'
