...
if @action_flag=3  
 begin   
  ...
  begin   
   select -1 error_code ,'Data exists for current hierarchy, Please contact Business Administrator.'  
   return  
  end   
  else   
  begin   
   delete from RuWorkFlow_detail  where RuWorkFlowHeader_id=@RuWorkFlowHeader_id  
   delete from RuWorkFlow_header  where RuWorkFlowHeader_id=@RuWorkFlowHeader_id  
  end  
 end   
...
select                  
  row_number() over (partition by 1 order by newid()) r  
...
BEGIN  
 ...
 inner join #xml_data x_data on  det.RuWorkFlowHeader_id=x_data.RuWorkFlowHeader_id  
 ...
 update det  
 set   
 RuWorkFlow_detail_levelId=x_data.level_id,  
...
,levels as(select u.RuWorkFlowHeader_id,u.MtWFHistory_Process_id,MtWFHistory_LevelID ,h.MtWFHistory_ToResource
			from update_To u inner join MtWFHistory h on u.RuWorkFlowHeader_id=h.RuWorkFlowHeader_id 
 ...
 from MtWFHistory h 
 inner join levels t on h.RuWorkFlowHeader_id=t.RuWorkFlowHeader_id
					and h.MtWFHistory_Process_id=t.MtWFHistory_Process_id
					and h.MtWFHistory_LevelID=t.MtWFHistory_LevelID
 inner join #xml_data x on h.RuWorkFlowHeader_id=x.RuWorkFlowHeader_id  
 ...
 ,@user_id  
 ,getutcdate()  
 from #xml_data where action_flag=1  
...
/*get all processes having chain count different from the current chain and reject them by system*/
 ...
 if exists(Select 1 from MtWFHistory h inner join #temp t on h.RuProcess_ID=t.RuProcess_ID and h.RuWorkFlowHeader_id=t.RuWorkFlowHeader_id
						where RuWorkFlowHeader_id=@RuWorkFlowHeader_id 
