/****** Object:  View [dbo].[ASC_CdpGuParties]    Committed by VersionSQL https://www.versionsql.com ******/
...
CREATE VIEW  [dbo].[ASC_CdpGuParties]
AS
...
SELECT  distinct
     ...
     ,cdp.FromPartyCategory_Code, cdp.ToPartyRegisteration_Id, cdp.ToPartyRegisteration_Name, cdp.ToPartyType_Code, cdp.ToPartyCategory_Code, cdp.RuCDPDetail_IsEnergyImported, ISNULL
			 ((SELECT TOP (1) 1 AS Expr1
                        ...
                        WHERE   (GU.MtGenerationUnit_Id in(select MtConnectedMeter_UnitId from MtConnectedMeter where  MtCDPDetail_Id=cdp.RuCDPDetail_Id)) AND (GU.SrTechnologyType_Code IN ('ARE', 'HYD'))), 0) AS IsARE, ISNULL
                      ((SELECT TOP (1) 1 AS Expr1
                        FROM      dbo.Bme_GuParties AS GU
                        WHERE   (GU.MtGenerationUnit_Id in(select MtConnectedMeter_UnitId from MtConnectedMeter where  MtCDPDetail_Id=cdp.RuCDPDetail_Id)) AND (GU.SrTechnologyType_Code = 'THR')), 0) AS IsThermal
