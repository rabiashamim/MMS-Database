/****** Object:  Procedure [dbo].[FCD_Execution_bk03072023]    Committed by VersionSQL https://www.versionsql.com ******/
...
Create   PROCEDURE dbo.FCD_Execution_bk03072023 @pMtFCDMaster_Id DECIMAL(18, 0)  
, @pUserId INT  
AS  
BEGIN  
 UPDATE MtFCDMaster  
...
/*  IF EXISTS (SELECT  
  ...
  ******************************************************************/  
  ...
  when Hourly.[MtFCDHourlyData_Generation]<=Gen.MtGenerator_TotalInstalledCapacity  
  and ISNULL(Hourly.[MtFCDHourlyData_Curtailment],0)=0   
  Then Hourly.[MtFCDHourlyData_Generation] 

  ----2. If Generation is greater than installed Net Capacity upto 5%  Then use Installed Net Capacity
  WHEN Hourly.[MtFCDHourlyData_Generation] > Gen.MtGenerator_TotalInstalledCapacity
    AND Hourly.[MtFCDHourlyData_Generation] <= (Gen.MtGenerator_TotalInstalledCapacity + (Gen.MtGenerator_TotalInstalledCapacity * 0.05))
    AND ISNULL(Hourly.[MtFCDHourlyData_Curtailment], 0) = 0
  THEN Gen.MtGenerator_TotalInstalledCapacity
   ...
   WHEN Hourly.[MtFCDHourlyData_Generation] > (Gen.MtGenerator_TotalInstalledCapacity + (Gen.MtGenerator_TotalInstalledCapacity * 0.05))
    AND ISNULL(Hourly.[MtFCDHourlyData_Curtailment], 0) = 0
  THEN  Gen.MtGenerator_TotalInstalledCapacity * Gen.MtFCDGenerators_EAFactor 
...
------1. (Curtailment)If Curtailment is less than or equal to Installed Net Capacity Then use Curtailment
  ...
  and isnull(Hourly.[MtFCDHourlyData_SOForecast],0)>=0  
  then Hourly.MtFCDHourlyData_Curtailment
  ...
  ---1. If Forecast is less than or equal to Installed Net Capacity Then use Forecast
   when Hourly.MtFCDHourlyData_SOForecast<=Gen.MtGenerator_TotalInstalledCapacity  
  and ISNULL(Hourly.[MtFCDHourlyData_Curtailment],0)=0   
  Then Hourly.[MtFCDHourlyData_Generation] 
  ----2.If Forecast is greater than installed Net Capacity upto 5%  Then use Installed Net Capacity
  when  Hourly.MtFCDHourlyData_SOForecast > Gen.MtGenerator_TotalInstalledCapacity
    AND Hourly.MtFCDHourlyData_SOForecast <= (Gen.MtGenerator_TotalInstalledCapacity + (Gen.MtGenerator_TotalInstalledCapacity * 0.05))
	 AND ISNULL(Hourly.[MtFCDHourlyData_Curtailment], 0) = 0
  then Gen.MtGenerator_TotalInstalledCapacity

  ----3.If Forecast is greater than installed Net Capacity above 5% Then use Installed Net Capacity * Factor
  WHEN Hourly.MtFCDHourlyData_SOForecast > (Gen.MtGenerator_TotalInstalledCapacity + (Gen.MtGenerator_TotalInstalledCapacity * 0.05))
    AND ISNULL(Hourly.[MtFCDHourlyData_Curtailment], 0) = 0
  THEN  Gen.MtGenerator_TotalInstalledCapacity * Gen.MtFCDGenerators_EAFactor
  end   
