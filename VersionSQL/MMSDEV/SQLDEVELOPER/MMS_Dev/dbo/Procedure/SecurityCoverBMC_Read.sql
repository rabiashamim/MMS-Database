/****** Object:  Procedure [dbo].[SecurityCoverBMC_Read]    Committed by VersionSQL https://www.versionsql.com ******/
...
CREATE PROCEDURE dbo.SecurityCoverBMC_Read @pMtSOFileMaster_Id DECIMAL(18, 0)  
, @pPageNumber INT  
, @pPageSize INT  
  ...
  BEGIN  
   ...
   SELECT  
    ...
    --      ,[MtBmcSecurityCover_RowNumber]        
      ...
      ,CASE  
     WHEN SC.MtSecurityCoverMP_IsValid = 1 THEN CAST(CAST(REPLACE(MtSecurityCoverMP_RequiredSecurityCover,',','') AS DECIMAL(18, 0)) AS NVARCHAR(MAX))  
     ELSE NULLIF([MtSecurityCoverMP_RequiredSecurityCover],'')  
    END AS MtSecurityCoverMP_RequiredSecurityCover  
    ,CASE  
     WHEN SC.MtSecurityCoverMP_IsValid = 1 THEN CAST(CAST(REPLACE(MtSecurityCoverMP_SubmittedSecurityCover,',','') AS DECIMAL(18, 0)) AS NVARCHAR(MAX))
     ELSE NULLIF([MtSecurityCoverMP_SubmittedSecurityCover] ,'') 
