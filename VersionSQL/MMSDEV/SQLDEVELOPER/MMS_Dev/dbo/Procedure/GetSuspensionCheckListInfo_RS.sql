/****** Object:  Procedure [dbo].[GetSuspensionCheckListInfo_RS]    Committed by VersionSQL https://www.versionsql.com ******/

-- GetSuspensionCheckListInfo  74        
        
CREATE PROCEDURE dbo.GetSuspensionCheckListInfo_RS           
...
WHERE       
    ...
    and MtRegisterationActivities_ACtion= case when @LuStatus_Code_Approval='SMDR' then 'SMDR' else 'SDRF'  end     
ORDER BY MtRegisterationActivity_Id DESC      
       select @MtRegisterationActivity_Id
Select           
...
FROM MtRegisterationActivities           
WHERE          
MtRegisterationActivities_ACtion = 'SMPA'        
AND ref_Id=@MtRegisterationActivity_Id        
order by MtRegisterationActivity_Id desc     
