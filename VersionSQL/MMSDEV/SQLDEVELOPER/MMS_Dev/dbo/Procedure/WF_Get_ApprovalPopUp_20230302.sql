/****** Object:  Procedure [dbo].[WF_Get_ApprovalPopUp_20230302]    Committed by VersionSQL https://www.versionsql.com ******/
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
         ...
         --WF_Get_ApprovalPopUp 303,18, 4,1                                
          --WF_Get_ApprovalPopUp 308,14, 4,1                                     
                                   --   EXEC WF_Get_ApprovalPopUp 318, 22,4,1        
...
--Comments :                                                                   
--======================================================================                                                             
CREATE procedure dbo.WF_Get_ApprovalPopUp_20230302                                                
...
SELECT  
 @modprocessid = RuModulesProcess_Id  
FROM RuModulesProcess  
WHERE RuModulesProcess_ProcessTemplateId = @RuModules_Id  
AND RuModules_Id = @settlementid  
...
SELECT  
 @RuNotificationSetup_ID = RuNotificationSetup_ID  
...
AND RuNotificationSetup_CategoryKey = CASE  
 WHEN ISNULL(@level_id, 0) = 1 THEN 'Process_Submitted'  
...
--SELECT  
-- @subject = STRING_AGG(CAST(RuModulesProcessDetails_ColumnName AS NVARCHAR(MAX)), ' | ')  
--FROM RuModulesProcessDetails  
--WHERE RuModulesProcess_Id = @modprocessid  
--AND RuModulesProcessDetails_IsSubject = 1  
  
select  @subject=STRING_AGG( CONCAT('cast(',RuModulesProcessDetails_ColumnName,' as NVARCHAR(MAX))' ),'+''|''+')  from RuModulesProcessDetails where RuModulesProcess_Id=@modprocessid and RuModulesProcessDetails_IsSubject=1   
set @subject=''+@subject+''  
...
--select @subject_query as [sub-query]  
  
  
--select @RuNotificationSetup_EmailSubject= CONCAT(''''+RuNotificationSetup_EmailSubject+ ' ' +@ProcessName+'''', ' +', @subject)  
--FROM RuNotificationSetup  
--WHERE RuNotificationSetup_CategoryKey = 'Submitted'  
...
SELECT  
 @RuNotificationSetup_EmailBody = CONCAT(@ProcessName, ' ', RuNotificationSetup_EmailBody)  
FROM RuNotificationSetup  
WHERE RuNotificationSetup_CategoryKey = 'Submitted'  
  
  
SELECT  
 @from_sql = STRING_AGG(CONCAT('<tr><td><b>', RuModulesProcessDetails_Label, ':</b></td><td>'' + cast(', ISNULL(RuModulesProcessDetails_ColumnName, ''), ' as NVARCHAR(MAX))+''</td></tr>'), '')  
FROM RuModulesProcessDetails  
WHERE RuModulesProcess_Id = @modprocessid  
...
SELECT  
 @RuWorkFlowHeader_id RuWorkFlowHeader_id  
  ...
  -- @modprocessid as RuModulesProcessID  
...
/*Registration*/  
IF @modprocessid BETWEEN 13 AND 18  
 OR @modprocessid = 20  
BEGIN  
